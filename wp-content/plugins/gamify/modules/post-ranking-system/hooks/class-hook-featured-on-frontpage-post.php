 ...
 * @version 1.0
 */
if ( ! class_exists( 'GFY_PRS_Hook_Featured_Front_Page_Post' ) && class_exists( 'myCRED_Hook' ) ) {

	class GFY_PRS_Hook_Featured_Front_Page_Post extends myCRED_Hook {

	    const REFERENCE = 'gfy_prs_featured_frontpage_post';

		/**
		 * GFY_PRS_Hook_Featured_Front_Page_Post constructor.
		 *
		...
		public function add_references( $references ) {
			...
			$references[ self::REFERENCE ] = __( 'Have featured post on front page', 'gamify' );
	...
	GFY_myCRED_Hook_Management_Service::get_instance()->register_hook(
		GFY_PRS_Hook_Featured_Front_Page_Post::REFERENCE,
		__( 'Points for making featured post on front page | Gamify', 'gamify' ),
		__( 'Award %_plural% to authors when post becomes featured on front page.', 'gamify' ),
		array( 'GFY_PRS_Hook_Featured_Front_Page_Post' )
	);
