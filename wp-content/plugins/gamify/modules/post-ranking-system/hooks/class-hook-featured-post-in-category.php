 ...
 */
if ( ! class_exists( 'GFY_PRS_Hook_Featured_Post_In_Category' )
	&& class_exists( 'myCRED_Hook' )
) {

	class GFY_PRS_Hook_Featured_Post_In_Category extends myCRED_Hook {

		const REFERENCE = 'gfy_prs_featured_post_in_category';

		/**
		 * GFY_PRS_Hook_Featured_Post_In_Category constructor.
		 *
		...
		function __construct( $hook_prefs, $type = MYCRED_DEFAULT_TYPE_KEY ) {
			...
			$defaults = array(
				'row_0' => array(
					'creds'     => 1,
					'category'  => 0,
					'log'       => '%plural% for featured post in %category%'
				)
			);
		...
		private function get_reference_label( $cat_name ) {
			return sprintf( __( 'Have featured post in "%s" category', 'gamify' ), $cat_name );
		}
	...
	GFY_myCRED_Hook_Management_Service::get_instance()->register_hook(
		GFY_PRS_Hook_Featured_Post_In_Category::REFERENCE,
		__( 'Points for featured post in categories | Gamify', 'gamify' ),
		__( 'Award %_plural% to authors when post from chosen categories becomes featured.', 'gamify' ),
		array( 'GFY_PRS_Hook_Featured_Post_In_Category' )
	);
