...
if ($connector != 'mymail' && $connector != 'mailster' && $connector != 'mailpoet' && $connector != 'conversio') {
	...
	if ($connector == 'mailchimp') {
	    essb5_draw_input_option('subscribe_mc_customtags2', esc_html__('Tags', 'essb'), esc_html__('Optional. Customize the global tags for this form design only. Leave blank to use the global (or if you are not using tags).', 'essb'), true);
	}
}
