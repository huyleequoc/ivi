...
function boombox_captcha_security_images( $width = 145, $height = 35, $characters = 6 ) {
	$font_path = __DIR__ . DIRECTORY_SEPARATOR . 'fonts' . DIRECTORY_SEPARATOR . 'calibri.ttf';
	...
	$textbox = imagettfbbox( $font_size, 0, $font_path, $code ) or die( esc_html__( 'Error in imagettfbbox function' ) );
	$x = ( $width - $textbox[4] ) / 2;
	$y = ( $height - $textbox[5] ) / 2;
	imagettftext( $image, $font_size, 0, $x, $y, $text_color, $font_path, $code ) or die( esc_html__( 'Error in imagettftext function' ) );
	...
	session_start();
	$_SESSION["boombox_contact_form_captcha_key"] = $code;
	session_write_close();
