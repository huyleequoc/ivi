<li class="zf-quiz_question zf-3up zf-poll-item  <?php if ( isset( $_COOKIE[ "zf_poll_vote_" . $question["question_id"] ] ) ) {
	echo 'zf-poll-done';
} ?>"
		...
		if ( isset( $question["answers"] ) ) {
			foreach ( $question["answers"] as $answer ) {
				...
				<li class="zf-answer-item <?php if ( ! isset( zf_array_values( $answer["image"] )[0]["attachment_id"] ) ) {
					...
					echo 'zf-selected';
				} ?>" data-voted="<?php echo isset( $zombify_poll_results["answers"][ $answer["answer_id"] ] ) ? $zombify_poll_results["answers"][ $answer["answer_id"] ] : 0 ?>" data-id="<?php echo $answer["answer_id"]; ?>" data-post-id="<?php the_ID() ?>" data-voted-group="<?php echo isset( $zombify_poll_results["groups"][ $question["question_id"] ] ) ? $zombify_poll_results["groups"][ $question["question_id"] ] : 0 ?>" data-group-id="<?php echo $question["question_id"]; ?>">
								...
								<figcaption class="zf-answer_credit">
									<cite>
										<a href="<?php echo $answer["image_credit"]; ?>"
										   class="zf-answer_credit"
										   target="_blank"
										   rel="nofollow noopener"><?php echo ( isset( $answer["image_credit_text"] ) && $answer["image_credit_text"] ) ? $answer["image_credit_text"] : __( 'Credit', 'zombify' ); ?></a>
									</cite>
								</figcaption>
							<?php } ?>
