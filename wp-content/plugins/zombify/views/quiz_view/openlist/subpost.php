		...
		<div class="zf-open_list zf-list">
			<div class="zf-list_item zf-single">
				...
				<div class="zf-item_meta">
                    <span class="zf-author"><span class="zf-author_avatar">
							<a href="<?php echo get_author_posts_url( $post->post_author ); ?>">
								<?php echo get_avatar( $post->post_author ); ?></a>
						</span>
					...
					if ( get_post_meta( $post->post_parent, "openlist_close_voting", true ) != 1 ) {
						?>
						<div class="zf-item-vote-box">
							<div class="zf-item-vote" data-zf-post-id="<?php echo $post->ID; ?>"
								 data-zf-post-parent-id="<?php echo $post->post_parent; ?>">
								<button class="zf-vote_btn zf-vote_up"><i class="zf-icon zf-icon-vote_up"></i>
								</button>
								<span class="zf-vote_count" data-zf-post-id="<?php echo $post->ID; ?>">
                                        <i class="zf-icon zf-spinner-pulse"></i>
                                        <span class="zf-vote_number">
                                            <?php echo (int) get_post_meta( $post->ID, "zombify_post_rateing", true ); ?>
                                        </span>
                                    </span>
				...
				<?php if ( $data["description"] ) { ?>
					<div class="zf-list_description"><?php echo $data["description"]; ?></div><?php } ?>
				...
				if ( $post->post_status == 'publish' ) {
					...
					<div class="zf-next-prev-pagination">
						...
						if ( $prev_data ) {
							?>
							<a class="zf-nav zf-prev" href="<?php echo get_permalink( $prev_data["post_id"] ); ?>"><span
										class="zf-icon zf-icon-angle-double-left"></span><?php esc_html_e( "Previous", "zombify" ); ?>
							</a>
							<?php
						}
						?>
						...
						if ( $next_data ) {
							?>
							<a class="zf-nav zf-next" href="<?php echo get_permalink( $next_data["post_id"] ); ?>"><span
										class="zf-icon zf-icon-angle-double-right"></span><?php esc_html_e( "Next", "zombify" ); ?>
							</a>
							<?php
						}
						?>
